...
| 27B served (OpenAI endpoint, DFlash2), 46 t/s on a 200-token code reply; 2 slots: 61 t/s aggregate for two concurrent code requests | `MODEL=$B27 DRAFT=models/Qwen3.8-27B-DFlash2-Q4_K_M.gguf tools/serve.sh --ctx 8192 --slots 2` then chat completions (docs/serving.md) | `timings.predicted_per_second ≈ 46` alone; two concurrent: 600 tokens in ~10 s wall |
