...
int main(int argc, char** argv) {
    ...
    for (int ct : {1, 2, 3}) {
        ...
        for (int variant = 0; variant < 1; ++variant) {
            auto run = [&] { hip::moe_gemm(fmt, hip::WFmt::Q8_0_SOA, dW, dW, ebytes, d_tiles, (int)tiles.size(), nullptr, 0, ct, d_rowtok, xq, dy, N, K, 0); };
            ...
            for (int i = 0; i < 8; ++i) { const int k = (int)((size_t)i * 977 % NS), te = order[k], e = eid[te], tk = te / NEXP, row = (int)((size_t)i * 131 % N);
                ...
                double r = 0, mag = 0; for (int kk = 0; kk < K; ++kk) { const double xv = hx[(size_t)tk * K + kk]; r += (double)wrow[kk] * xv; mag += fabs((double)wrow[kk] * xv); }
            ...
            printf("  int8 CT=%d: %4zu tiles  %8.2f ms  %6.1f TOPS  weights %.2f GB -> %5.0f GB/s per pass  maxrel %.1e %s\n", ct, tiles.size(), ms, flop / ms / 1e9, bytes / 1e9, bytes / ms / 1e6, maxrel, maxrel < 1e-2 ? "OK" : "MISMATCH");
        }
    }
    return 0;
}
