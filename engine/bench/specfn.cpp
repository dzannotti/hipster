// specfn <gguf> <ref.json> [n_gen] [n_draft] : greedy generation with MTP speculative decoding.
...
int main(int argc, char** argv) {
    ...
    const int n_gen = argc > 3 ? atoi(argv[3]) : 64;
    std::vector<int> n_drafts; { std::stringstream ss(argc > 4 ? argv[4] : "3"); std::string tok; while (std::getline(ss, tok, ',')) n_drafts.push_back(atoi(tok.c_str())); }
    hip::Qwen4Exp m(argv[1]);
    const int V = hip::FnDims::n_vocab, E = hip::FnDims::wide;
    ...
    { m.reset(); int pos = 0;
      ...
      double dt = now_ms() - t0;
      printf("plain greedy: %d tokens in %.0f ms = %.2f t/s (forward GPU %.0f ms, PLE host gather %.0f ms)\n", n_gen, dt, n_gen * 1000.0 / dt, m.gpu_ms(), m.ple_host_ms()); }

    if (getenv("HIPSTER_VERIFY_PROF")) {   // time T-token verify passes on the plain stream (profiling aid)
        const int T = atoi(getenv("HIPSTER_VERIFY_PROF"));
        m.reset(); int pos = 0; for (int t : prompt) { m.forward(&t, 1, pos++); m.accept(1); }
        double gsum = 0; int n = 0;
        for (int i = 0; i + T <= n_gen; i += T) { const float ms = m.forward(&plain[i], T, pos); m.accept(T); pos += T; if (i > 0) { gsum += ms; ++n; } }
        printf("verify T=%d: %.1f ms GPU per pass (%d passes)\n", T, gsum / n, n);
        return 0;
    }
    ...
    // ---- MTP speculative greedy, once per draft length ----
    int rc = 0;
    for (int n_draft : n_drafts) {
    std::vector<int> spec;
    ...
    { m.reset(); int pos = 0;
      ...
      const double gpu0 = m.gpu_ms(), ple0 = m.ple_host_ms();
      ...
      double dt = now_ms() - t0;
      spec.resize(n_gen);
      printf("mtp n=%d: %d tokens in %.0f ms = %.2f t/s | %d rounds, acceptance %d/%d = %.0f%%, %.2f tokens/round | draft %.0f ms, verify %.0f ms (GPU %.0f), catch-up %.0f ms, PLE host %.0f ms\n",
             n_draft, n_gen, dt, n_gen * 1000.0 / dt, rounds, accepted, drafted, 100.0 * accepted / drafted, (double)(accepted + rounds) / rounds, t_draft, t_verify, m.gpu_ms() - gpu0, t_catch, m.ple_host_ms() - ple0); }
