// dflash27b <gguf> <draft.gguf> <ref.json> [n_gen] [n_draft<=7] : greedy generation with the DFlash2 block draft.
...
int main(int argc, char** argv) {
    ...
    { m.reset();
      ...
      int rounds = 0, drafted = 0, accepted = 0; std::vector<int> hist(n_draft + 1, 0);
      double t0 = now_ms(), t_draft = 0, t_verify = 0, t_enc = 0;
      while ((int)spec.size() < n_gen) {
          int drafts[8]; double ta = now_ms();
          const int nd = m.dflash_draft(id_last, pos, n_draft, drafts);
